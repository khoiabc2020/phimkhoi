...
import { IS_TV_APP } from '../tv/config';
import { TvHeroCard } from '../tv/TvHeroCard';

const { width, height } = Dimensions.get('window');
const isTablet = width > 700;
const isTvLike = IS_TV_APP || Platform.isTV;

const POSTER_WIDTH = isTvLike ? width * 0.35 : (isTablet ? width * 0.4 : width * 0.6);
const POSTER_HEIGHT = POSTER_WIDTH * 1.30;
const CAROUSEL_HEIGHT = POSTER_HEIGHT + (isTvLike ? 120 : 180);
    ...
    return (
        <View style={styles.carouselSection}>
            <Carousel
                loop
                width={width}
                height={(IS_TV_APP || Platform.isTV) ? TV_CAROUSEL_HEIGHT : CAROUSEL_HEIGHT}
                data={movies}
                autoPlay={true}
                autoPlayInterval={5500}
                onSnapToItem={onSnapToItem}
                scrollAnimationDuration={320}
                mode={(IS_TV_APP || Platform.isTV) ? undefined : 'parallax'}
                modeConfig={(IS_TV_APP || Platform.isTV) ? undefined : { parallaxScrollingScale: 0.92, parallaxScrollingOffset: 40 }}

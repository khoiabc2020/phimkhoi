...
async function main() {
    ...
    if (toNotify.length > 0) {
        ...
        // Tránh tạo duplicate notification cùng userId+slug+episode trong vòng 12h
        ...
        const existingKeys = new Set(
            ...
            }, 'userId movieSlug').lean()).map(n => `${n.userId}:${n.movieSlug}`)
        );

        const newNotifs = notifDocs.filter(n => !existingKeys.has(`${n.userId}:${n.movieSlug}`));

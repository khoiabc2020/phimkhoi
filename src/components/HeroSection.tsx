...
function getHeroDescription(movie: any) {
    const description = decodeHtml(
        stripHtml(movie?.content || movie?.description || movie?.tmdbData?.overview || "")
    ).trim();
    ...
    const categories = Array.isArray(movie?.category)
        ? movie.category
              .map((item: any) => decodeHtml(item?.name || "").trim())
              .filter(Boolean)
              .slice(0, 3)
        : [];

    if (categories.length > 0) {
        return `Khám phá ${categories.join(", ").toLowerCase()} trong bộ phim ${decodeHtml(
            movie?.name || movie?.origin_name || "nổi bật"
        )}.`;
    }

    return `Khám phá bộ phim ${decodeHtml(movie?.name || movie?.origin_name || "nổi bật")} đang được quan tâm trên KHOIPHIM.`;
}
...
function hasDesktopHeroBackdrop(movie: any) {
    ...
    const tmdb = shouldUseTmdbMedia(movie, movie?.tmdbData) ? movie.tmdbData : null;
    return Boolean(tmdb?.backdrop_path || getStrictLandscapeFromMovie(movie));
}
...
function HeroBackdropFrame({ src, alt, priority = false }: { src: string; alt: string; priority?: boolean }) {
...
function HeroThumbTile({
    ...
    const title = decodeHtml(movie?.name || movie?.origin_name || "Phim");
...
function MobileHero({ movies, active = true }: { movies: Movie[]; active?: boolean }) {
    ...
    useEffect(() => {
        ...
        [movies[index], movies[(index + 1) % movies.length], movies[(index - 1 + movies.length) % movies.length]]
            .map((item) => item?.slug)
            .filter(Boolean)
            .forEach((slug) => router.prefetch(`/phim/${slug}`));
    }, [index, movies, router]);
    ...
    return (
        ...
        >
                ...
                >
                    <div className="absolute inset-0">
                        <div className="absolute inset-0 opacity-40 blur-3xl scale-110 pointer-events-none">
                            <Image
                                src={getImageUrl(getHeroImage(movie, "backdrop", "mobile") || getHeroImage(movie, "poster", "mobile") || "/placeholder.jpg")}
                                alt=""
                                fill
                                className="object-cover"
                                quality={54}
                        ...
                        <Image
                            src={getImageUrl(getHeroImage(movie, "poster", "mobile") || getHeroImage(movie, "backdrop", "mobile") || "/placeholder.jpg")}
                            alt=""
                            fill
                            className="object-cover"
                            priority={index === 0}
            ...
            <div className="absolute bottom-6 left-0 right-0 z-20 flex justify-center gap-1.5 px-4 overflow-hidden">
                {movies.slice(0, 10).map((_, tileIndex) => (
                    <button
                        key={tileIndex}
                        onClick={() => go(tileIndex)}
                        className={cn(
                            "h-1.5 rounded-full transition-all duration-300",
                            tileIndex === index ? "w-6 bg-[#8FA7C5] shadow-[0_0_8px_#8FA7C5]" : "w-1.5 bg-white/30"
                        )}
                    />
                ))}
            </div>
...
function DesktopHero({ movies, active = true }: { movies: Movie[]; active?: boolean }) {
    ...
    useEffect(() => {
        ...
        const activeEl = container.children[index] as HTMLElement | undefined;
        if (!activeEl) return;
    ...
    return (
        ...
        >
                        ...
                        <div className="w-full flex justify-between items-end">
                            <div className="w-full md:w-[75%] lg:w-[65%] xl:w-[60%] space-y-3 lg:space-y-4 pointer-events-auto pr-0 lg:pr-[300px] xl:pr-[400px]">
                                ...
                                <div className="flex items-center gap-2 flex-wrap pt-1">
                                    {movie.category?.slice(0, 3).map((category: any) => (
                                        <Link
            ...
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-[30] w-full max-w-[90vw] md:max-w-3xl lg:max-w-5xl">
                ...
                >
                    {movies.map((heroMovie: any, heroIndex) => (
                        <HeroThumbTile
                            key={`hero-thumb-${heroMovie._id || heroIndex}`}
                            movie={heroMovie}
                            active={heroIndex === index}
                            onClick={() => go(heroIndex)}
                        />
                    ))}
                </div>
            </div>
            ...
            <div className="absolute top-[35%] md:top-[40%] -translate-y-1/2 left-0 right-0 z-[35] pointer-events-none flex items-center justify-between px-2 md:px-8 lg:pl-32 lg:pr-10 xl:pl-40 xl:pr-16">
                <button
                    onClick={prev}
                    className="h-8 w-8 md:h-9 md:w-9 lg:h-10 lg:w-10 rounded-full bg-transparent border border-transparent flex items-center justify-center text-white/15 hover:text-white/45 hover:bg-white/[0.04] active:scale-95 transition-all duration-300 pointer-events-auto"
        ...
        <div className="relative w-full bg-[#0a0a0a] font-sans" style={{ minHeight: "400px", contain: "layout transition", willChange: "transform" }}>
            {!isDesktop && (
                <div className="md:hidden">
                    <MobileHero movies={mobileMovies} active />
                </div>
            )}

            {isDesktop && desktopMovies.length > 0 && (
                <div className="hidden md:block">
                    <DesktopHero movies={desktopMovies} active />
                </div>
            )}
        </div>
    );

            ...
            <div className="flex flex-wrap gap-4 pt-1">
                {cast.map((actor) => (
                    <Link
                        ...
                        title={actor.name}
                    >
                        <div className="w-14 h-14 rounded-full overflow-hidden shrink-0 border border-white/15 group-hover:border-[#E50914] transition-colors relative bg-white/5 ring-1 ring-white/5 group-hover:ring-[#E50914]/40">
                            {actor.photo ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {cast.map((actor) => (
                    <Link
                        href={`/dien-vien/${encodeURIComponent(actor.name)}`}
                        key={actor.name}
                        className="relative flex items-center gap-3 bg-white/5 rounded-lg p-3 group hover:bg-white/10 transition-colors border border-white/5 hover:border-[#E50914]/60"
                    >
                        <div className="relative w-16 h-16 rounded-full overflow-hidden border-2 border-white/10 group-hover:border-[#E50914] transition-colors shrink-0">
                            {actor.photo ? (
                                <Image src={actor.photo} alt={actor.name} fill className="object-cover" unoptimized />
                            ) : (
                                <ActorInitials name={actor.name} />
                        ...
                        <span className="hidden sm:inline text-[11px] font-semibold text-gray-400 group-hover:text-[#E50914] transition-colors ml-2">
                            Xem trang
                        </span>
                    </Link>
                ))}
